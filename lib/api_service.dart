...
class ApiService {
  ...
  static Future<List<SharedFileEntry>> getAllSharedFiles() async {
    try {
      ...
      final url = Uri.parse('$baseUrl${AppConfig.allFilesEndpoint}');
      ...
      print('Get All Shared Files API Response Status: ${response.statusCode}');
      print('Get All Shared Files API Response Body: ${response.body}');
      ...
      if (response.statusCode == 200) {
        ...
        if (responseData.containsKey('data')) {
          ...
          if (data is List) {
            ...
            filesList = [data]; // Single file object, wrap in array
          } else {
            filesList = [];
          }
          ...
          if (userRole == 'doctor') {
            // For doctors: show files where they are the doctor
            filteredFiles = filesList
                .where((file) => file['doctorId'] == userId)
                .toList();
          } else {
            // For patients: show all files (if needed)
            filteredFiles = filesList;
          }

          return filteredFiles.map((file) => SharedFileEntry.fromJson(file)).toList();
        } else {
          // Fallback: if response doesn't have 'data' key, return empty list
          return <SharedFileEntry>[];
        }
      } else {
        throw ApiException('Failed to fetch shared files');
      }
    } catch (e) {
      if (e is ApiException) {
        rethrow;
      }
      print('Get All Shared Files API Error: $e');
      throw ApiException('Network error. Please check your connection.');
    }
  }

  // Add log entry when doctor views a file
  static Future<bool> addViewLog({
    required String doctorId,
    required String doctorName,
    required String patientId,
    required String fileName,
    ...
    try {
      ...
      final url = Uri.parse('$baseUrl${AppConfig.addLogEndpoint}');
      ...
      if (response.statusCode == 200 || response.statusCode == 201) {
        ...
        print('Failed to add log: ${response.body}');
        return false; // Don't throw error for logging failures
      }
    } catch (e) {
      print('Add Log API Error: $e');
      return false; // Don't throw error for logging failures
  ...
  static Future<bool> deleteSharedFile(String fileId) async {
    try {
      final token = await TokenService.getToken();
      if (token == null) {
        throw ApiException('Authentication required');
      ...
      final url = Uri.parse('$baseUrl${AppConfig.deleteSharedFileEndpoint(fileId)}');
      ...
      print('Delete Shared File API Response Status: ${response.statusCode}');
      print('Delete Shared File API Response Body: ${response.body}');

      if (response.statusCode == 200 || response.statusCode == 204) {
        return true;
      } else {
        throw ApiException('Failed to delete shared file');
      }
    } catch (e) {
      if (e is ApiException) {
        rethrow;
      }
      print('Delete Shared File API Error: $e');
      throw ApiException('Network error. Please check your connection.');
    }
  }
}
...
class SharedFileEntry {
  ...
  factory SharedFileEntry.fromJson(Map<String, dynamic> json) {
    return SharedFileEntry(
      id: json['_id']?.toString() ?? json['id']?.toString() ?? '',
      doctorId: json['doctorId'] ?? '',
      doctorName: json['doctorName'] ?? '',
      patientId: json['patientId'] ?? '',
      patientName: json['patientName'] ?? 'Patient ${json['patientId']?.toString().substring(0, 8) ?? 'Unknown'}',
      fileName: json['fileName'] ?? '',
      fileUrl: json['fileUrl'] ?? '',
  ...
  // Helper method to format the share date for display
  String get formattedDate {
    try {
      final dateTime = DateTime.parse(shareDate.isNotEmpty ? shareDate : createdAt);
      return '${dateTime.day}/${dateTime.month}/${dateTime.year}';
    } catch (e) {
      return shareDate.isNotEmpty ? shareDate : createdAt;
    }
  }

  // Helper method to format the share time for display
  ...
  // Helper method to format the last accessed date for display
  String get formattedLastAccessed {
    try {
      final dateTime = DateTime.parse(lastAccessed);
